...
extension TodolistPresenter {
  ...
  func configureSection(_ cell: TodolistHeaderCell, forSectionAt section: Int) {
    let sectionModel = sections[section]
    switch sectionModel {
    case let .delayed(headerItem, _):
      if case let .header(cellModel) = headerItem {
        cell.configure(cellModel)
      }
    case let .todo(headerItem, _):
      if case let .header(cellModel) = headerItem {
        cell.configure(cellModel)
      }
    }
...
//  }
  ...
  func didChangedComplete(indexPath: IndexPath, completed: Bool) {
    let item = sections[indexPath.section].items[indexPath.row]
    if case let .todo(cellModel) = item {
//      cellModel
...
//        self.setTodolist(sections: sections)
