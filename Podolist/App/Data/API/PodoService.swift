...
class PodoService: PodoServiceProtocol {
    ...
    func getAllPodolist() -> Observable<[ResponsePodo]> {
        return Observable<[ResponsePodo]>.create { observer in
            let request = Alamofire.request(Router.Podolist.get(params: "")).validate().responseJSON { response in
    ...
    func getPodo(podoId: Int) -> Observable<ResponsePodo> {
        return Observable<ResponsePodo>.create { observer in
            let request = Alamofire.request(Router.Podolist.get(params: String(podoId))).validate().responseJSON { response in
                switch response.result {
                case .success(let value):
                    if let statusCode = response.response?.statusCode, statusCode == 200 {
